...
pub fn try_host_from_aws_region(query: &str) -> Result<Host, AWSRegionNotFoundError> {
    ...
    match region {
        None => Err(AWSRegionNotFoundError {
...
mod tests {
    ...
    fn test_host_from_city() {
        assert_eq!(
            try_host_from_aws_region("aws:Singapore"),
            Ok("ec2.ap-southeast-1.amazonaws.com".to_string())
        );
    }
    #[test]
    fn test_host_from_imaginary_city() {
        assert_eq!(
            try_host_from_aws_region("Atlantis"),
            Err(AWSRegionNotFoundError {
    ...
    fn test_host_from_region_name() {
        assert_eq!(
            try_host_from_aws_region("aws:cn-north-1"),
            Ok("ec2.cn-north-1.amazonaws.com".to_string())
        );
    }
}
